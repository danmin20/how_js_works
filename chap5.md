...
유리수는 두 개의 정수의 비율로 나타낼 수 있는 수이다.  
큰 정수, 2진 부동소수점 수, 10진 부동소수점 수, 그 외의 다른 형에서 정확하게 표현할 수 없는 유리수 등을 정확하게 표현한다.

유리수 시스템은 두 개의 숫자, 분자(numerator)와 분모(denominator)로 구성된다.  
`값 = 분자/분모`  
이런 방식이 산술 연산에 적합하다.  
분모는 항상 양수여야 한다.

big_rational 라이브러리를 통해 자바스크립트에서는 불가능했던 계산을 할 수 있다.  
그러므로 자바스크립트에 새로운 숫자형을 추가할 필요는 없다.
